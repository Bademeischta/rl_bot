...
#   powershell -ExecutionPolicy Bypass -File tools\apply_patches.ps1 [-Check]
#
...
$Repo = "$Root\third_party\RLGymPPO_CPP"
$Patches = @(
...
if ($head -ne $Pinned) {
    Write-Host "WARNUNG: third_party\RLGymPPO_CPP steht auf $head, gepinnt ist $Pinned (PINNED.md)." -ForegroundColor Yellow
}
...
foreach ($patch in $Patches) {
    ...
    git -C $Repo apply --check --reverse $patch 2>$null
